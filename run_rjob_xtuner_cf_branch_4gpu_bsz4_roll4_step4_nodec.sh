...
# 4-GPU rjob: XTuner CoGRPO v2 cf_branch (non-decoupled: verifier=actor, no verifier LoRA),
...
if [ -z "${xtuner_branch}" ] || [ "${xtuner_branch}" != "${xtuner_branch_expected}" ]; then
  echo "[xtcf4-nodec] ERROR: expected xtuner branch ${xtuner_branch_expected}, got ${xtuner_branch:-<none>}" >&2
  exit 1
fi

exp_name="${EXP_NAME:-xtcf4-nodec-bsz4-roll4-step4-$(date +%m%d%H%M)}"
...
# Debug dumps / prints (enabled by default in debug phase).
...
echo "[xtcf4-nodec] xtuner=${xtuner_branch}@${xtuner_commit} dir=${xtuner_dir}"
echo "[xtcf4-nodec] name=${rjob_name} model=${model_path}"
echo "[xtcf4-nodec] dataset=${dataset_path}"
echo "[xtcf4-nodec] rollout_steps=${rollout_steps} gbs=${global_batch_size} opt_steps=${train_optimizer_steps} prk=${prompt_repeat_k}"
echo "[xtcf4-nodec] reward_urls=${reward_model_urls}"
echo "[xtcf4-nodec] COMPASSVERIFIER_DEBUG=${compass_debug} max_calls=${compass_debug_max_calls}"
