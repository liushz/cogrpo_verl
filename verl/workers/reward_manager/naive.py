...
class NaiveRewardManager:
    ...
    def __init__(
        ...
        try:
            reward_cfg = (
                getattr(config, "reward_model", None) if config is not None else None
            )
            if reward_cfg is not None:
                ...
                if hasattr(reward_cfg, "get"):
                    ...
                    key = (
                        reward_cfg.get("reward_model_key")
                        or reward_cfg.get("key")
                        or key
                    )
                else:
                    cfg_urls = (
                    ...
                    key = (
                        getattr(reward_cfg, "reward_model_key", None)
                        or getattr(reward_cfg, "key", None)
                        or key
                    )
                if cfg_urls:
        ...
        self.reward_model_clients = (
            [OpenAI(base_url=url, api_key=key) for url in urls] if urls else None
        )
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        if data.non_tensor_batch:
            ...
            for key, val in list(data.non_tensor_batch.items()):
                if isinstance(val, np.ndarray):
                    if val.shape == ():
                        data.non_tensor_batch[key] = np.full(
                            (batch_size,), val.item(), dtype=object
                        )
                    elif val.shape[0] != batch_size:
                        if val.size == 1:
                            data.non_tensor_batch[key] = np.full(
                                (batch_size,), val.reshape(-1)[0], dtype=object
                            )
                    ...
                    if len(val) == batch_size:
                        ...
                        data.non_tensor_batch[key] = np.full(
                            (batch_size,), val[0], dtype=object
                        )
                else:
                    data.non_tensor_batch[key] = np.full(
                        (batch_size,), val, dtype=object
                    )
        ...
        if debug_enabled:
            print(
                f"[DEBUG NaiveRewardManager] batch keys: {list(data.batch.keys())}",
                file=sys.stderr,
            )
            print(
                f"[DEBUG NaiveRewardManager] non_tensor_batch keys: {list(data.non_tensor_batch.keys())}",
                file=sys.stderr,
            )
            print(
                f"[DEBUG NaiveRewardManager] reward_model_clients: {self.reward_model_clients}",
                file=sys.stderr,
            )
            print(f"[DEBUG NaiveRewardManager] len(data): {len(data)}", file=sys.stderr)
            print(
                f"[DEBUG NaiveRewardManager] data.batch['responses'].shape: {data.batch['responses'].shape}",
                file=sys.stderr,
            )
            if hasattr(data.batch, "get") and "attention_mask" in data.batch:
                print(
                    f"[DEBUG NaiveRewardManager] data.batch['attention_mask'].shape: {data.batch['attention_mask'].shape}",
                    file=sys.stderr,
                )
        ...
        def process_row(args):
            ...
            def _to_jsonable_scalar(v):
                ...
                if isinstance(v, np.ndarray):
                    if v.size == 1:
                        return (
                            v.reshape(-1)[0].item()
                            if isinstance(v.reshape(-1)[0], np.generic)
                            else v.reshape(-1)[0]
                        )
                    return v.tolist()
            ...
            if "prompts" not in data_item.batch:
                if debug_enabled and (debug_limit <= 0 or i < debug_limit):
                    print(
                        f"[DEBUG] Row {i}: 'prompts' key not in data_item.batch! Available keys: {list(data_item.batch.keys())}",
                        file=sys.stderr,
                    )
                ...
                if "input_ids" in data_item.batch:
                    if debug_enabled and (debug_limit <= 0 or i < debug_limit):
                        print(
                            f"[DEBUG] Row {i}: Using 'input_ids' as prompts fallback",
                            file=sys.stderr,
                        )
                    ...
                    if debug_enabled and (debug_limit <= 0 or i < debug_limit):
                        print(
                            f"[DEBUG] Row {i}: ERROR - No 'input_ids' either!",
                            file=sys.stderr,
                        )
            ...
            if debug_enabled and (debug_limit <= 0 or i < debug_limit):
                print(
                    f"[DEBUG Row {i}] prompt_ids.shape: {prompt_ids.shape}",
                    file=sys.stderr,
                )
                print(
                    f"[DEBUG Row {i}] prompt_length: {prompt_length}", file=sys.stderr
                )
                print(
                    f"[DEBUG Row {i}] valid_prompt_length: {valid_prompt_length}",
                    file=sys.stderr,
                )
                print(
                    f"[DEBUG Row {i}] attention_mask.shape: {attention_mask.shape}",
                    file=sys.stderr,
                )
                print(
                    f"[DEBUG Row {i}] response_ids.shape: {response_ids.shape}",
                    file=sys.stderr,
                )
                print(
                    f"[DEBUG Row {i}] attention_mask sum: {attention_mask.sum()}",
                    file=sys.stderr,
                )
                print(
                    f"[DEBUG Row {i}] attention_mask[prompt_length:] sum: {attention_mask[prompt_length:].sum()}",
                    file=sys.stderr,
                )
                print(
                    f"[DEBUG Row {i}] response_ids (first 100): {response_ids[:100]}",
                    file=sys.stderr,
                )
                print(
                    f"[DEBUG Row {i}] response_ids (min value): {response_ids.min()}",
                    file=sys.stderr,
                )
                print(
                    f"[DEBUG Row {i}] response_ids (max value): {response_ids.max()}",
                    file=sys.stderr,
                )
                print(
                    f"[DEBUG Row {i}] tokenizer vocab_size: {self.tokenizer.vocab_size}",
                    file=sys.stderr,
                )
            ...
            if debug_enabled and (debug_limit <= 0 or i < debug_limit):
                print(
                    f"[DEBUG Row {i}] valid_response_length: {valid_response_length}",
                    file=sys.stderr,
                )
                print(
                    f"[DEBUG Row {i}] valid_response_ids.shape: {valid_response_ids.shape}",
                    file=sys.stderr,
                )
                print(
                    f"[DEBUG Row {i}] valid_response_ids (first 100): {valid_response_ids[:100]}",
                    file=sys.stderr,
                )
            ...
            full_response_with_hints = self.tokenizer.decode(
            ...
            stream_type_from_trainer = data_item.non_tensor_batch.get(
                "stream_type", None
            )
            ...
            if stream_type_from_trainer:
                ...
                if not has_exp_metadata and critiques_val:
                    if isinstance(critiques_val, str):
                        has_exp_metadata = (
                            critiques_val.strip() != ""
                            and critiques_val != "No interventions"
                        )
                    elif isinstance(critiques_val, np.ndarray):
                        has_exp_metadata = critiques_val.size > 0 and any(
                            c and c != "No interventions" for c in critiques_val
                        )

                # Also check num_interventions as additional signal
                num_interventions = data_item.non_tensor_batch.get(
            ...
            if debug_enabled and (debug_limit <= 0 or i < debug_limit):
                ...
                print(
                    f"[DEBUG Row {i}] is_exp_stream: {is_exp_stream}", file=sys.stderr
                )
            ...
            if debug_enabled and (debug_limit <= 0 or i < debug_limit):
                ...
                print(
                    f"[DEBUG Row {i}] response_str (first 200 chars): {response_str[:200]}",
                    file=sys.stderr,
                )
                print(f"[DEBUG Row {i}] ground_truth: {ground_truth}", file=sys.stderr)
                print(
                    f"[DEBUG Row {i}] reward_model_clients: {self.reward_model_clients}",
                    file=sys.stderr,
                )
            ...
            if debug_enabled and (debug_limit <= 0 or i < debug_limit):
                print(
                    f"[DEBUG Row {i}] score result: {score} (type: {type(score)})",
                    file=sys.stderr,
                )
            ...
            # Attach reward/label into dump for offline debugging.
            try:
                dump_record["reward"] = float(reward)
                if reward in (0, 1) or reward in (0.0, 1.0):
                    dump_record["label"] = int(reward)
                    dump_record["is_correct"] = bool(int(reward))
            except Exception:
                pass
        ...
        for (
            ...
            reward_position = max(
                0, valid_response_length - 1
            )  # Clamp to 0 if response is empty
            reward_position = min(
                reward_position, reward_tensor.size(1) - 1
            )  # Clamp to max tensor size
        ...
        if self.config is not None and hasattr(self.config, "trainer"):
            dual_rollout_dump_freq_config = self.config.trainer.get(
                "dual_rollout_dump_freq", None
            )
            if dual_rollout_dump_freq_config is not None:
                ...
                rollout_dump_freq_config = self.config.trainer.get(
                    "rollout_dump_freq", 0
                )
        ...
        dump_base_dir = work_dir or os.environ.get(
            "VERL_DUMP_DIR", "/mnt/shared-storage-user/liuhongwei/main_works/temp_debug"
        )
