...
def _post_process_outputs(tokenizer, output):
    def _map_each_response(resp):
        ...
        input_log_probs, input_token_ids = zip(*[(log_prob if log_prob is not None else 0.0, token_ids) for log_prob, token_ids, _ in input_token_logprobs])
        return torch.tensor(output_token_ids), torch.tensor(log_probs), torch.tensor(input_token_ids), torch.tensor(input_log_probs), resp["meta_info"]["finish_reason"]["type"]

    out_map = map(lambda x: _map_each_response(x), output)
    batched_output_token_ids = []
    batched_logprobs = []
    batched_input_token_ids = []
    batched_input_log_probs = []
    batched_truncation_mask = []
    for output_token_ids, log_probs, input_token_ids, input_log_probs, finish_reason in out_map:
        batched_output_token_ids.append(output_token_ids)
        batched_logprobs.append(log_probs)
        batched_input_token_ids.append(input_token_ids)
        batched_input_log_probs.append(input_log_probs)
        if finish_reason == "length":
            batched_truncation_mask.append(1.0)
        else:
            batched_truncation_mask.append(0.0)
    pad_token_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else tokenizer.eos_token_id
    batched_output_token_ids = pad_sequence(batched_output_token_ids, batch_first=True, padding_value=pad_token_id)
    if len(batched_logprobs) > 0:
        batched_logprobs = pad_sequence(batched_logprobs, batch_first=True, padding_value=pad_token_id)
    batched_truncation_mask = torch.tensor(batched_truncation_mask)
    return batched_output_token_ids, batched_logprobs, batched_input_token_ids, batched_input_log_probs, batched_truncation_mask
...
class SGLangRollout(BaseRollout):
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        only_get_input_log_probs = prompts.meta_info.get("only_get_input_log_probs", False)
        if only_get_input_log_probs:
            kwargs = dict(
                n=1,
                temperature=1.0,
                ignore_eos=False,
        ...
        with self.update_sampling_params(**kwargs):
            ...
            input_ids = out[2]
            input_log_probs = out[3]
            input_ids = pad_2d_list_to_length(input_ids, self.pad_token_id, idx.size(1), left=True).to(idx.device)
            input_log_probs = pad_2d_list_to_length(input_log_probs, self.pad_token_id, idx.size(1), left=True).to(idx.device)

            truncation_mask = out[4]
