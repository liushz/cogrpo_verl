...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        response_mask = data.batch["response_mask"]
        advantages, returns = (
            core_algos.compute_repro_reinforce_plus_plus_outcome_advantage(
                token_level_rewards=data.batch["token_level_rewards"],
                token_level_repro_rewards=data.batch["token_level_repro_rewards"],
                response_mask=response_mask,
                repro_mask=data.batch["repro_mask"],
        ...
        response_mask = data.batch["response_mask"]
        advantages, returns = (
            core_algos.compute_repro_reinforce_plus_plus_baseline_outcome_advantage(
                token_level_rewards=data.batch["token_level_rewards"],
                token_level_repro_rewards=data.batch["token_level_repro_rewards"],
                response_mask=response_mask,
                repro_mask=data.batch["repro_mask"],
        ...
        # Control stream mask
        control_response_mask = data.batch.get(
            "control_response_mask", data.batch["response_mask"]
        )
        ...
        control_token_level_rewards = data.batch.get(
            "control_token_level_rewards", None
        )
        exp_token_level_rewards = data.batch.get(
            "exp_token_level_rewards", data.batch["token_level_rewards"]
        )

        # Get verifier rewards (relative advantage: r_B - r_A)
        verifier_token_level_rewards = data.batch.get(
            "verifier_token_level_rewards", None
        )
        verifier_response_mask = data.batch.get("verifier_response_mask", None)

        if config is None:
            raise ValueError(
                "config is required for CO_GRPO. Make sure self.config.algorithm is properly initialized."
            )
        ...
        if hasattr(config, "get"):
            ...
            control_group_weight = (
                getattr(config, "control_group_weight", 0.5)
                if hasattr(config, "control_group_weight")
                else 0.5
            )
        ...
        (
            ...
            verifier_returns,
            metadata,
        ) = core_algos.compute_co_grpo_advantage(
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.parallel_control_exp = bool(
            self.config.trainer.get("parallel_control_exp", False)
        )
    ...
    def _validate_config(self):
        config = self.config
        if (
            self.parallel_control_exp
            and config.actor_rollout_ref.actor.strategy not in ["fsdp", "fsdp2"]
        ):
            raise NotImplementedError(
                "parallel_control_exp only supports fsdp/fsdp2 strategies"
            )
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            if (
                self.parallel_control_exp
                and self.config.actor_rollout_ref.rollout.mode != "sync"
            ):
                raise NotImplementedError(
                    "parallel_control_exp only supports sync rollout mode"
                )
            ...
            actor_rollout_ref_config = OmegaConf.to_container(
            ...
            if self.parallel_control_exp:
                control_pool = self.resource_pool_manager.get_resource_pool(
                    Role.ActorRolloutControl
                )
                ...
                self.resource_pool_to_cls[control_pool]["actor_rollout_control"] = (
                    control_rollout_cls
                )
        else:
            raise NotImplementedError
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        ...
                        if use_co_grpo:
                            ...
                            sample_uids = np.array(
                                [
                                    str(uuid.uuid4())
                                    for _ in range(len(gen_batch_repeated))
                                ],
                                dtype=object,
                            )
                            gen_batch_repeated.non_tensor_batch["sample_uid"] = (
                                sample_uids
                            )
                            ...
                            if use_parallel:
                                if self.actor_rollout_control_wg is None:
                                    raise RuntimeError(
                                        "parallel_control_exp enabled but control rollout worker group is missing"
                                    )
                                ...
                                freeze_actor = bool(
                                    self.config.get("verifier", {}).get(
                                        "freeze_actor", False
                                    )
                                )
                                control_sync_freq = int(
                                    self.config.trainer.get(
                                        "control_rollout_sync_freq", 1
                                    )
                                )
                                should_sync_control = (
                                    ...
                                    and (
                                        self.global_steps == 0
                                        or is_last_step
                                        or self.global_steps % control_sync_freq == 0
                                    )
                                )
                                if should_sync_control:
                                    t0 = time.time()
                                    control_state = (
                                        self.actor_rollout_wg.export_actor_state_dict()
                                    )
                                    if isinstance(control_state, list):
                                        control_state = next(
                                            (state for state in control_state if state),
                                            None,
                                        )
                                    if control_state is None:
                                        raise RuntimeError(
                                            "Failed to export actor state dict for control rollout sync"
                                        )
                                    ...
                                    try:
                                        ...
                                        self.actor_rollout_control_wg.load_actor_state_dict(
                                            control_state_ref
                                        )
                                    except Exception:
                                    ...
                                    del control_state
                                    t1 = time.time()
                                    metrics["control_rollout/sync_freq"] = float(
                                        control_sync_freq
                                    )
                                ...
                                exp_future = (
                                    self.actor_rollout_wg.dual_stream_rollout_async(
                                        gen_batch_repeated,
                                ...
                                control_timing = control_output.meta_info.get(
                                    "timing", {}
                                )
                                ...
                                control_output = self._co_grpo_convert_control_output(
                                    control_output
                                )
                                gen_batch_output = control_output.union(exp_output)
                                if verifier_batch is not None:
                                    gen_batch_output.meta_info["verifier_batch"] = (
                                        verifier_batch
                                    )
                    ...
                    if use_co_grpo:
                        ...
                        if (
                            ...
                            # Exclude padding tokens (EOS mask alone keeps trailing PADs when EOS is absent).
                            try:
                                pad_id = int(self.tokenizer.pad_token_id)
                                batch.batch["control_response_mask"] = batch.batch[
                        ...
                        if "exp_loss_mask" in batch.batch:
                            ...
                            batch.batch["response_mask"] = batch.batch["exp_loss_mask"][
                                :, -exp_resp_len:
                            ]
                        else:
                    ...
                    if use_co_grpo:
                        try:
                            ...
                            if "exp_loss_mask" in batch.batch:
                                exp_policy_lens = (
                                    batch.batch["exp_loss_mask"][:, -exp_resp_len:]
                                    .to(torch.float32)
                                    .sum(dim=1)
                                )
                                metrics["co_grpo/exp_policy_len_mean"] = (
                                    exp_policy_lens.mean().item()
                                )
                                metrics["co_grpo/exp_hint_len_mean"] = (
                    ...
                    if use_co_grpo:
                        batch.meta_info["global_token_num"] = torch.sum(
                            batch.batch["exp_attention_mask"], dim=-1
                        ).tolist()
                    else:
                        batch.meta_info["global_token_num"] = torch.sum(
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        if use_co_grpo:
                            ...
                            control_batch.batch["attention_mask"] = (
                                control_batch.batch.pop("control_attention_mask")
                            )
                            control_batch.batch["position_ids"] = (
                                control_batch.batch.pop("control_position_ids")
                            )
                            ...
                            if "exp_prompts" in batch.batch.keys():
                                ...
                                exp_batch.batch["prompts"] = exp_batch.batch[
                                    "input_ids"
                                ][..., :exp_prompt_len]
                            # Mark stream type for reward manager dump
                            # CRITICAL FIX: Must be a list, not a scalar, to avoid IndexError in protocol.py:275
                            exp_batch.non_tensor_batch["stream_type"] = [
                                "exp"
                            ...
                            from verl.trainer.ppo.reward import (
                                compute_relative_advantage_reward,
                            )
                            ...
                            raw_relative_rewards = compute_relative_advantage_reward(
                                control_rewards=control_outcome, exp_rewards=exp_outcome
                            )

                            # Optional: intervention penalty + reward shaping need intervention stats.
                            num_interventions = batch.non_tensor_batch.get(
                                "num_interventions", None
                            )
                            hint_token_counts = batch.non_tensor_batch.get(
                                "hint_token_counts", None
                            )
                            ...
                            num_interventions = _safe_int_array(
                                num_interventions, "num_interventions"
                            )
                            hint_token_counts = _safe_int_array(
                            ...
                            if reward_mode == "headroom":
                                ...
                                control_clamped = torch.clamp(
                                    control_outcome, min=0.0, max=1.0
                                )
                                headroom = torch.clamp(
                                    1.0 - control_clamped, min=headroom_min
                                )
                                clamped_gap = torch.clamp(
                                    raw_relative_rewards / headroom, min=-1.0, max=1.0
                                )
                                metrics["co_grpo/verifier_reward_headroom_mean"] = (
                                    float(headroom.mean().item())
                                )
                            else:
                                if reward_mode != "gap":
                                    logger.warning(
                                ...
                                clamped_gap = torch.clamp(
                                    raw_relative_rewards, min=-1.0, max=1.0
                                )
                            ...
                            improve_coef = 1.0
                            if hasattr(
                                self.config.algorithm, "verifier_reward_weighting"
                            ):
                                weight_config = (
                                    self.config.algorithm.verifier_reward_weighting
                                )
                                improve_coef = float(
                                    weight_config.get("improve_coef", 1.0)
                                )

                            verifier_outcome_rewards = (
                                clamped_gap + improve_coef * positive_gap
                            )

                            # Log boost factor (even though reward uses additive boost).
                            reward_weight = 1.0 + improve_coef * positive_gap
                            metrics["co_grpo/verifier_reward_weight_mean"] = float(
                                reward_weight.mean().item()
                            )
                            metrics["co_grpo/verifier_reward_weight_improve_ratio"] = (
                                float((raw_relative_rewards > 0).float().mean().item())
                            )

                            # Apply a weak penalty only when there is no improvement.
                            intervention_costs = torch.zeros_like(
                                verifier_outcome_rewards
                            )
                            if hasattr(self.config.algorithm, "intervention_penalty"):
                                ...
                                if (
                                    lambda_freq > 0 or lambda_len > 0
                                ) and num_interventions is not None:
                                    device = verifier_outcome_rewards.device
                                    cost = torch.zeros_like(verifier_outcome_rewards)
                                    num_interventions_tensor = torch.as_tensor(
                                        num_interventions,
                                        device=device,
                                        dtype=verifier_outcome_rewards.dtype,
                                    )
                                    ...
                                    verifier_outcome_rewards = torch.where(
                                        clamped_gap > 0,
                                        verifier_outcome_rewards,
                                        verifier_outcome_rewards - cost,
                                    )

                                    # Log intervention statistics
                                    metrics["co_grpo/avg_interventions"] = np.mean(
                                        num_interventions
                                    )
                                    metrics["co_grpo/avg_hint_tokens"] = (
                                        np.mean(hint_token_counts)
                                        if hint_token_counts is not None
                                        else 0.0
                                    )
                                    metrics["co_grpo/avg_intervention_cost"] = (
                                        intervention_costs.mean().item()
                                    )
                                    metrics["co_grpo/max_interventions"] = np.max(
                                        num_interventions
                                    )
                                    metrics["co_grpo/min_interventions"] = np.min(
                                        num_interventions
                                    )

                            # Log reward comparison statistics
                            metrics["co_grpo/control_reward_mean"] = (
                                control_outcome.mean().item()
                            )
                            metrics["co_grpo/control_reward_std"] = (
                                control_outcome.std().item()
                            )
                            metrics["co_grpo/exp_reward_mean"] = (
                                exp_outcome.mean().item()
                            )
                            metrics["co_grpo/exp_reward_std"] = exp_outcome.std().item()

                            # Log relative reward statistics (before penalty)
                            metrics["co_grpo/relative_reward_mean"] = (
                                raw_relative_rewards.mean().item()
                            )
                            metrics["co_grpo/relative_reward_std"] = (
                                raw_relative_rewards.std().item()
                            )
                            metrics["co_grpo/relative_reward_positive_ratio"] = (
                                (raw_relative_rewards > 0).float().mean().item()
                            )

                            # Log verifier reward after penalty
                            metrics["co_grpo/verifier_reward_mean"] = (
                                verifier_outcome_rewards.mean().item()
                            )
                            metrics["co_grpo/verifier_reward_std"] = (
                                verifier_outcome_rewards.std().item()
                            )
                            ...
                            improvement_ratio = torch.clamp(
                                improvement_ratio, min=-5.0, max=5.0
                            )
                            metrics["co_grpo/improvement_ratio_mean"] = (
                                improvement_ratio.mean().item()
                            )
                            # Log improvement ratio distribution for better monitoring
                            metrics["co_grpo/improvement_ratio_median"] = (
                                improvement_ratio.median().item()
                            )
                            metrics["co_grpo/improvement_ratio_std"] = (
                                improvement_ratio.std().item()
                            )
                            metrics["co_grpo/improvement_ratio_min"] = (
                                improvement_ratio.min().item()
                            )
                            metrics["co_grpo/improvement_ratio_max"] = (
                                improvement_ratio.max().item()
                            )
                            # Log control reward zero ratio (detect control stream failures)
                            control_zero_ratio = (
                                (control_outcome.abs() < 0.01).float().mean().item()
                            )
                            metrics["co_grpo/control_zero_ratio"] = control_zero_ratio

                            # Expand to token level for verifier
                            batch.batch["verifier_token_level_rewards"] = (
                                verifier_outcome_rewards.unsqueeze(-1).expand(
                                    -1, exp_resp_len
                                )
                            )
                            batch.batch["verifier_response_mask"] = batch.batch[
                                "exp_response_mask"
                            ]
                            ...
                            if verifier_batch is not None and len(verifier_batch) > 0:
                                try:
                                    ...
                                    debug_asserts = (
                                        ...
                                        or os.getenv("VERL_LOGGING_LEVEL", "").upper()
                                        == "DEBUG"
                                    )
                                    if (
                                        debug_asserts
                                        and int(os.environ.get("RANK", "0")) == 0
                                        and verifier_batch.batch is not None
                                    ):
                                        for key, tensor in verifier_batch.batch.items():
                                            if isinstance(tensor, torch.Tensor):
                                                assert tensor.device.type == "cpu", (
                                                    f"verifier_batch.{key} must be on CPU, got {tensor.device}"
                                                )
                                    ...
                                    if (
                                        ...
                                        raise ValueError(
                                            "verifier_batch missing required non_tensor fields: parent_uid/parent_sample_uid/step_idx"
                                        )
                                    ...
                                    if batch_sample_uids is None:
                                        raise ValueError(
                                            "batch missing non_tensor_batch['sample_uid'] for mapping verifier rewards"
                                        )

                                    verifier_outcome_cpu = (
                                        verifier_outcome_rewards.detach().to("cpu")
                                    )
                                    reward_by_sample_uid = {}
                                    for i in range(len(batch_sample_uids)):
                                        reward_by_sample_uid[
                                            str(batch_sample_uids[i])
                                        ] = verifier_outcome_cpu[i]

                                    # Count interventions per sample_uid for credit assignment.
                                    count_by_sample_uid = {}
                                    for su in parent_sample_uids:
                                        su = str(su)
                                        count_by_sample_uid[su] = (
                                            count_by_sample_uid.get(su, 0) + 1
                                        )
                                    ...
                                    for j in range(len(verifier_batch)):
                                        ...
                                        r_total = reward_by_sample_uid.get(
                                            su, torch.tensor(0.0)
                                        )
                                        denom = max(
                                            1, int(count_by_sample_uid.get(su, 0))
                                        )
                                        step_rewards.append(r_total / denom)
                                        group_index.append(f"{gu}:{step_idx}")

                                    step_rewards = torch.stack(step_rewards, dim=0).to(
                                        torch.float32
                                    )  # (m,)
                                    response_len = int(
                                    ...
                                    token_level_rewards = (
                                        step_rewards.unsqueeze(-1) / token_counts
                                    ...
                                    verifier_batch.non_tensor_batch["step_reward"] = (
                                        np.array(
                                            [float(r.item()) for r in step_rewards],
                                            dtype=np.float32,
                                        )
                                    )
                                    batch.meta_info["verifier_train_batch"] = (
                                        verifier_batch
                                    )
                                except Exception as e:
                                    logger.warning(
                            ...
                            if self.config.reward_model.launch_reward_fn_async:
                                ...
                                future_reward = compute_reward_async.remote(
                                    batch, self.config, tokenizer_name_or_path
                                )
                            else:
                    ...
                    with _timer("old_log_prob", timing_raw):
                        ...
                        if use_co_grpo:
                            ...
                            log_prob_batch.batch["input_ids"] = (
                                log_prob_batch.batch.pop("exp_input_ids")
                            )
                            log_prob_batch.batch["attention_mask"] = (
                                log_prob_batch.batch.pop("exp_attention_mask")
                            )
                            log_prob_batch.batch["position_ids"] = (
                                log_prob_batch.batch.pop("exp_position_ids")
                            )
                            log_prob_batch.batch["responses"] = batch.batch[
                                "exp_responses"
                            ]
                    ...
                    if self.config.algorithm.adv_estimator in [
                        ...
                        with _timer("repro_score", timing_raw):
                            ...
                            valid_prompt_len = batch.batch["attention_mask"][
                                :, :prompt_len
                            ].sum(1)
                            valid_response_len = batch.batch["attention_mask"][
                                :, prompt_len:
                            ].sum(1)
                            device = batch.batch.device

                            end_think_token_id = self.tokenizer.encode(
                                "</think>", add_special_tokens=False
                            )[0]
                            think_mask = (
                                ~(batch.batch["responses"] == end_think_token_id)
                                .cumsum(1)
                                .bool()
                            )
                            ...
                            forking_token_indices = F.pad(
                                forking_token_indices, (1, 0), value=0
                            )
                            forking_token_indices = torch.cat(
                                [
                                    forking_token_indices,
                                    (think_mask.sum(1, keepdim=True) + 1).clamp_max(
                                        valid_response_len.unsqueeze(1)
                                    ),
                                ],
                                dim=1,
                            )

                            batch_idxs = torch.arange(forking_token_indices.size(0)).to(
                                forking_token_indices.device
                            )
                            batch_idxs = einops.repeat(
                                batch_idxs, "b -> b n", n=forking_token_indices.size(1)
                            )

                            forking_token_indices = (
                                torch.stack([batch_idxs, forking_token_indices], dim=2)
                                .view(-1, 2)
                                .unique(dim=0)
                            )

                            aux_batch = deepcopy(batch)
                            aux_batch = aux_batch.sample_level_repeat(
                                forking_token_indices[:, 0].bincount()
                            )
                            ...
                            prob_token_lens = torch.tensor(
                                list(map(len, prob_tokens))
                            ).to(entropys.device)

                            max_prob_token_len = prob_token_lens.max()
                            indices = (
                                torch.arange(max_prob_token_len)
                                .unsqueeze(0)
                                .to(entropys.device)
                            )
                            pad_token_id = self.tokenizer.pad_token_id
                            ...
                            aux_batch.batch["position_ids"] = torch.cat(
                                [
                                    ...
                                    indices
                                    + aux_batch.batch["position_ids"][:, -1:]
                                    + 1,
                                ],
                                dim=1,
                            )

                            seq_indices = torch.arange(seq_len + max_prob_token_len).to(
                                entropys.device
                            )
                            seq_indices = einops.repeat(
                                seq_indices, "s -> b s", b=len(forking_token_indices)
                            )

                            cols = [
                                torch.arange(l).to(entropys.device)
                                for l in prob_token_lens
                            ]
                            cols = torch.cat(cols, dim=0)
                            cols = (
                                cols
                                + forking_token_indices[:, 1].repeat_interleave(
                                    prob_token_lens
                                )
                                + prompt_len
                            )
                            rows = torch.arange(len(forking_token_indices)).to(
                                entropys.device
                            )
                            ...
                            r = (
                                forking_token_indices[:, 1]
                                + prob_token_lens
                                + prompt_len
                            )
                            p_mask = seq_indices >= r.unsqueeze(1)
                            v_mask = (seq_indices >= prompt_len) & (
                                seq_indices < r.unsqueeze(1)
                            )
                            aux_batch.batch["input_ids"][p_mask] = (
                            ...
                            aux_log_prob = self.actor_rollout_wg.compute_log_prob(
                                aux_batch
                            )

                            repro_value_mask = (seq_indices >= l.unsqueeze(1)) & (
                                seq_indices < r.unsqueeze(1)
                            )
                            ...
                            repro_value = -(
                                aux_log_prob.batch["old_log_probs"] * repro_value_mask
                    ...
                    if self.use_reference_policy:
                        ...
                        with _timer("ref", timing_raw):
                            if use_co_grpo:
                                ...
                                ref_batch.batch["input_ids"] = ref_batch.batch.pop(
                                    "exp_input_ids"
                                )
                                ref_batch.batch["attention_mask"] = ref_batch.batch.pop(
                                    "exp_attention_mask"
                                )
                                ref_batch.batch["position_ids"] = ref_batch.batch.pop(
                                    "exp_position_ids"
                                )
                                ref_batch.batch["responses"] = batch.batch[
                                    "exp_responses"
                                ]
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if use_co_grpo:
                            ...
                            reward_extra_infos_dict: dict[str, list] = (
                                reward_extra_infos_dict
                                if "reward_extra_infos_dict" in locals()
                                else {}
                            )
                        else:
                            reward_extra_infos_dict: dict[str, list]
                        ...
                        if self.config.algorithm.adv_estimator in [
                            ...
                            def kendall_batch(x_batch, y_batch):
                                ...
                                x_batch_expanded_i = x_batch.unsqueeze(
                                    2
                                )  # Adds a dimension at position 2
                                x_batch_expanded_j = x_batch.unsqueeze(
                                    1
                                ...
                                diff_x = (
                                    x_batch_expanded_i - x_batch_expanded_j
                                ).sign()
                                diff_y = (
                                    y_batch_expanded_i - y_batch_expanded_j
                                ).sign()
                            ...
                            repro_scores = torch.stack(
                                [
                                    torch.cat([x, x.new_zeros(max_len - len(x))])
                                    for x in repro_scores
                                ]
                            )
                            ...
                            repro_scores = (
                                repro_scores[:, 0:1] - repro_scores[:, 1:]
                            ) / (repro_scores[:, 0:1] + 1e-6)
                            ...
                            for i in range(1, ema_repro_scores.size(1)):
                                x = (
                                    torch.arange(1, i + 2)
                                    .unsqueeze(0)
                                    .repeat(len(ema_repro_scores), 1)
                                    .to(ema_repro_scores.device)
                                )
                                mono_scores.append(
                                    -kendall_batch(x, ema_repro_scores[:, : i + 1])
                                    + 1 / 2
                                )
                            ...
                            batch.batch["token_level_repro_rewards"] = torch.zeros_like(
                                batch.batch["token_level_scores"]
                            )
                            batch.batch["token_level_repro_rewards"][
                                groups, values - 1
                            ] = repro_scores[repro_scores_mask]

                            batch.batch["repro_mask"] = torch.zeros_like(
                                batch.batch["token_level_scores"]
                            )
                        ...
                        training_progress = min(
                            1.0, self.global_steps / self.total_training_steps
                        )
                        ...
                        if use_co_grpo and "co_grpo_metadata" in batch.meta_info:
                            ...
                            metrics["co_grpo/training_progress"] = co_grpo_meta[
                                "training_progress"
                            ]
                            metrics["co_grpo/effective_control_weight"] = co_grpo_meta[
                                "effective_control_weight"
                            ]
                            metrics["co_grpo/effective_exp_weight"] = (
                                1.0 - co_grpo_meta["effective_control_weight"]
                            )

                            # Log advantage statistics for both streams
                            if "advantages" in batch.batch:
                                policy_advantages = batch.batch["advantages"]
                                metrics["co_grpo/policy_advantage_mean"] = (
                                    policy_advantages.mean().item()
                                )
                                metrics["co_grpo/policy_advantage_std"] = (
                                    policy_advantages.std().item()
                                )

                            if "verifier_advantages" in batch.batch:
                                verifier_advantages = batch.batch["verifier_advantages"]
                                metrics["co_grpo/verifier_advantage_mean"] = (
                                    verifier_advantages.mean().item()
                                )
                                metrics["co_grpo/verifier_advantage_std"] = (
                                    verifier_advantages.std().item()
                                )
                    ...
                    if self.config.trainer.save_freq > 0 and (
                        ...
                        with _timer("save_checkpoint", timing_raw):
                            ...
                            try:
                                verifier_cfg = (
                                    getattr(self.config, "verifier", {}) or {}
                                )
                                verifier_lora_enabled = (
                                    int(verifier_cfg.get("lora_rank", 0)) > 0
                                )
                            except Exception:
                            ...
                            if (
                                ...
                                and (
                                    is_last_step
                                    or self.global_steps % verifier_lora_save_freq == 0
                                )
                                ...
                                os.makedirs(
                                    os.path.dirname(verifier_ckpt_dir), exist_ok=True
                                )
                                t0 = time.time()
                                self.actor_rollout_wg.save_verifier_lora(
                                    verifier_ckpt_dir
                                ...
                                t1 = time.time()
                                metrics["verifier/lora_ckpt_save_freq"] = float(
                                    verifier_lora_save_freq
                                )
