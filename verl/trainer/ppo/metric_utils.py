...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    Returns:
        A dictionary of metrics including:
            ...
            - co_grpo/control_reward/mean, max, min: Control stream rewards (if Co-GRPO)
            - co_grpo/exp_reward/mean, max, min: Experimental stream rewards (if Co-GRPO)
            - co_grpo/verification_advantage/mean, max, min: Relative advantage r_B - r_A (if Co-GRPO)
            - co_grpo/verifier_advantages/mean, max, min: Verifier advantages (if Co-GRPO)
    """
    ...
    if is_co_grpo:
        ...
        metrics.update({
            "co_grpo/verification_advantage/mean": torch.mean(verification_advantage).detach().item(),
            "co_grpo/verification_advantage/max": torch.max(verification_advantage).detach().item(),
...
def calc_maj_val(data: list[dict[str, Any]], vote_key: str, val_key: str) -> float:
    ...
    for d in data:
        # NOTE: If vote is a list, it will cause "unhashable type: 'list'" error
        # This should be fixed at the source (reward_manager) level, not here
        # vote = d[vote_key]
        # if isinstance(vote, list):
        #     vote = tuple(vote) if vote else ""
        # vote2vals[vote].append(d[val_key])
